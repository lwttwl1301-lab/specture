"""
多峰光谱重建训练 V3.2.1 - 方案 2.1：改进的混合损失函数
基于方案二失败教训的改进版本

改进点：
1. 前 550 轮只用 MSE 训练，建立基础曲线重建能力
2. 最后 50 轮加入小权重 (0.02) 峰位置损失
3. 避免峰位置损失干扰前期 MSE 优化
"""
...
print('=' * 70)
print('多峰光谱训练 V3.2.1 - 方案 2.1（小权重 + 后期加入）')
print('=' * 70)
...
# ============================================================================
# 7. 训练 - 改进的混合损失函数（方案 2.1）
# ============================================================================
print('\n[7/8] 训练（方案 2.1：小权重 + 后期加入）...')
...
def peak_position_loss(pred, true_peak_positions, wavelengths_tensor):
    """
...
def combined_loss(pred, target_spectra, true_peak_positions, weights, wavelengths_tensor, 
                  mse_weight=1.0, peak_weight=0.02):
    """
    混合损失：MSE + 峰位置损失
    方案 2.1: 小权重 (0.02)，只在后期加入
    """
    mse = weighted_mse_loss(pred, target_spectra, weights)
    peak_loss = peak_position_loss(pred, true_peak_positions, wavelengths_tensor)
    
    # 归一化峰位置损失（使其与 MSE 量级相当）
    peak_loss_normalized = peak_loss / 100.0  # 峰位置误差通常在 0-100nm 范围
...
print('\n  训练策略:')
print('    阶段 1 (0-550 轮): 纯 MSE 训练')
print('    阶段 2 (550-600 轮): MSE + 峰位置损失 (权重 0.02)')
print()
...
for epoch in range(epochs):
    ...
    # 方案 2.1: 前 550 轮只用 MSE，最后 50 轮加入小权重峰位置损失
    use_peak_loss = (epoch >= 550)
    peak_w = 0.02 if use_peak_loss else 0.0
    ...
    for batch_X, batch_y, batch_peak_pos in train_loader:
        ...
        if use_peak_loss:
            loss, mse, peak_loss = combined_loss(
                ...
                mse_weight=1.0, peak_weight=peak_w
            )
        else:
            # 纯 MSE 训练
            mse = weighted_mse_loss(pred, batch_y, weights_tensor)
            loss = mse
    ...
    if (epoch + 1) % 100 == 0 or epoch == 549 or epoch == 550:
        status = "MSE+Peak" if use_peak_loss else "MSE only"
        print(f'  Epoch {epoch+1}, Loss: {epoch_loss:.6f} (MSE:{epoch_mse:.6f}, Peak:{epoch_peak:.2f}nm), Test: {test_loss:.6f} [{status}]')
...
print(f'  <1nm 精度：{np.sum(peak_position_errors < 1) / len(peak_position_errors) * 100:.1f}%')
print(f'  <2nm 精度：{np.sum(peak_position_errors < 2) / len(peak_position_errors) * 100:.1f}%')
print(f'  <5nm 精度：{np.sum(peak_position_errors < 5) / len(peak_position_errors) * 100:.1f}%')
...
ax.axvline(x=550, color='red', linestyle='--', alpha=0.5, label='Peak loss added')
ax.set_title('Training Curves (方案 2.1)')
ax.legend()
...
ax.axvline(x=550, color='red', linestyle='--', alpha=0.5)
...
plt.tight_layout()
plt.savefig('multi_peak_training_v2_1_results.png', dpi=150)
print('已保存：multi_peak_training_v2_1_results.png')
...
torch.save({
    ...
    'multi_peak': True,
    'version': 'v3.2.1_peak_loss_late',
}, 'model_multi_peak_v2_1.pth')

print('已保存：model_multi_peak_v2_1.pth')

# ============================================================================
# 11. 最终总结
# ============================================================================
print('\n' + '=' * 70)
print('多峰训练 V3.2.1 总结 - 方案 2.1（小权重 + 后期加入）')
print('=' * 70)

print(f"""
╔══════════════════════════════════════════════════════════════════════╗
║         方案 2.1：小权重 + 后期加入峰位置损失训练结果                  ║
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  【训练策略】                                                         ║
║    阶段 1 (0-550 轮): 纯 MSE 训练                                     ║
║    阶段 2 (550-600 轮): MSE + Peak(0.02)                            ║
║                                                                      ║
║  【曲线级别指标】                                                     ║
...
║    <1nm 精度：{np.sum(peak_position_errors < 1) / len(peak_position_errors) * 100:.1f}%                              ║
║    <2nm 精度：{np.sum(peak_position_errors < 2) / len(peak_position_errors) * 100:.1f}%                             ║
║    <5nm 精度：{np.sum(peak_position_errors < 5) / len(peak_position_errors) * 100:.1f}%                             ║
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝
""")
